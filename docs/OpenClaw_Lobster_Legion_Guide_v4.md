...
## 附录 C：OpenHarness 架构整合（v4.0 新增）

在 v4.0 版本中，我们深度整合了香港大学数据科学实验室（HKUDS）发布的 **OpenHarness** 框架的核心理念。OpenHarness 提出了一个被业界长期忽视但至关重要的概念：**Agent Harness（智能体 Harness）**。

> "The model is the agent. The code is the harness." （模型即智能体，代码即 Harness。）

### 1. 什么是 Agent Harness？

在 LLM 能力突飞猛进的今天，模型本身已经具备了强大的推理和决策能力，但如何将这些能力安全、高效、可扩展地转化为实际可用的生产力工具，需要一个完整的"Harness"——一套围绕 LLM 构建的基础设施，赋予它"手、眼、记忆和安全边界"。

一个能够真正自主完成任务的 AI Agent 需要：
- **感知能力**：读取文件、浏览网页、执行命令
- **行动能力**：写入文件、修改代码、调用 API
- **记忆能力**：记住上下文、持久化知识、跨会话学习
- **安全边界**：权限控制、操作审计、异常拦截
- **协调能力**：多 Agent 协作、任务分配、进度管理

这些能力并非 LLM 本身所具备，而是需要一套外部基础设施来提供。这就是 Agent Harness 的使命。

### 2. OPT 龙虾军团的 Harness 升级

在 v4.0 中，我们将 OPT 系统从一个单纯的"多 Agent 团队"升级为一个完整的 **Agent Harness 基础设施**：

#### 2.1 动态技能加载与 Hooks 机制
借鉴 OpenHarness 的设计，OPT 的技能系统（`SKILL.md`）不再仅仅是静态的提示词，而是包含了可执行的 Hooks 逻辑（如执行前的权限检查、执行后的结果清洗）。技能支持按需动态加载，避免一次性将所有技能塞入上下文，极大地降低了 Token 消耗。

#### 2.2 多级权限与安全审计
我们在 `Ops`（运维审计员）角色中引入了更严格的路径规则和命令拦截机制。任何涉及文件写入或系统命令执行的操作，都必须经过明确的权限校验或 `CoS`（幕僚长）的确认。这确保了 Agent 的行为始终在可控边界内。

#### 2.3 自动上下文压缩（Auto-Compact）
结合 OpenHarness 的 Auto-Compact 机制，我们进一步优化了 OPT 的记忆系统。在现有的三层记忆基础上，引入了自动摘要和关键信息提取流程，确保在长时间的复杂任务中，核心上下文不丢失，同时有效控制 Token 成本。

#### 2.4 Swarm 模式的多 Agent 协调
OPT 系统内置了多 Agent 协调机制，支持主 Agent（如 `CoS`）生成专门的子 Agent 处理特定任务、管理团队协作关系以及执行后台任务。这种 Swarm 模式能够显著提升并行任务的执行效率，是应对复杂工程项目的关键能力。

通过整合 OpenHarness 的核心架构理念，OPT 龙虾军团系统变得更加安全、高效、可扩展，真正实现了从"模型中心"向"基础设施中心"的范式转移。
